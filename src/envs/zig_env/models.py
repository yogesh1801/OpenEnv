...
"""
Data models for the Zig Environment.

The Zig environment executes Zig code and provides feedback through
...
class ZigAction(Action):
    """
    Action for the Zig environment - code to execute.
    
    Attributes:
        core_code: Core Zig code to execute
...
@dataclass(kw_only=True)
class ZigObservation(Observation):
    """
    Observation from the Zig environment - execution results.
...
@dataclass
class ZigState(State):
    """
    State for Zig environment.
