...
"""
Ruby Environment HTTP Client.

This module provides the client for connecting to a Ruby Environment server
over HTTP.
"""
...
class RubyEnv(HTTPEnvClient[RubyAction, RubyObservation]):
    """
    HTTP client for the Ruby Environment.
    
    This client connects to a RubyEnvironment HTTP server and provides
    ...
    Example with Docker:
        ...
        >>> result = client.reset()
        >>> result = client.step(RubyAction(core_code='puts 2 + 2', test_code=''))
        >>> print(result.observation.stdout)  # "4\n"
        >>> client.close()
    """

    def _step_payload(self, action: RubyAction) -> Dict:
        """
        Convert RubyAction to JSON payload for step request.
        
        Args:
            action: RubyAction instance
    ...
    def _parse_result(self, payload: Dict) -> StepResult[RubyObservation]:
        """
        Parse server response into StepResult[RubyObservation].
        
        Args:
            payload: JSON response from server
            
        Returns:
            StepResult with RubyObservation
        """
        ...
        return StepResult[RubyObservation](
            observation=observation,
            reward=payload.get("reward"),
            done=payload.get("done", False),
        )

    def _parse_state(self, payload: Dict) -> RubyState:
        """
        Parse server response into RubyState object.
        
        Args:
            payload: JSON response from /state endpoint
            
        Returns:
            RubyState object with episode metadata
        """
        return RubyState(
